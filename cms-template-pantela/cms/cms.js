(() => {
  ...
  const PAGES_URL = '/cms/pages.json';
  ...
  async function loadPagesConfig() {
    try {
      const response = await fetch(`${PAGES_URL}?t=${Date.now()}`, { cache: 'no-store' });
      if (!response.ok) return;
      const json = await response.json();
      const groups = Array.isArray(json) ? json : json.groups;
      const safeGroups = normalizePagesConfig(groups);
      if (safeGroups.length) sitePages = safeGroups;
    } catch {}
  }

  function normalizePagesConfig(groups) {
    if (!Array.isArray(groups)) return [];
    return groups.map((group) => {
      const items = Array.isArray(group?.items) ? group.items : [];
      return {
        label: String(group?.label || 'Страницы'),
        items: items
          .map((item) => ({
            path: normalizePagePath(item?.path),
            label: String(item?.label || item?.path || 'Страница'),
          }))
